                ...
                if (po.All_outbound_wires_inserted__c)
                {
                    ...
                    foreach (purchaseOrderItem poi in metalItems)
                    {
                            ...
                            .Contains(poi.Coin_Metal_Type__c))
                        {
                            ...
                            .Contains(poi.Coin_Metal_Type__c)) 
                        {
                            allNames[lastVendor].Add(poi.Coin_Metal_Type__c);
                        }
                        if (poi != null &&
                            poi.Vendor__r != null && 
                            poi.Promo_coin__c &&
                    ...
                    foreach(KeyValuePair<string, List<string>> kvp in allNames)
                    {
                        ...
                        List<string>> kvp in vendorNames)
                    {
                        if (promoVendorNumber.Length > 0 &&
                            ...
                            !poNums.Contains(promoVendorNumber) &&
                            kvp.Value.Count > 0)
                        {
                            if (kvp.Value[0] == "Fee" && 
                                kvp.Value.Count == 1)
                        ...
                        notPromoNames)
                    {
                        if (thisVendorNumber.Length > 0 &&
                            ...
                            !eqbpo.ContainsKey(thisVendorNumber) &&
                            !poNums.Contains(thisVendorNumber) &&
                            kvp.Value.Count > 0)
                        {
