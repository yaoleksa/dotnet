...
{
    public class GetPurchaseOrderQB
    {
                    ...
                    for(int j = 0; j < retLst.Count; j++)
                    {
                            ...
                            retLst.GetAt(j).RefNumber.GetValue() != null)
                        {
                                ...
                                .GetValue()))
                            {
                                ...
                                if(retLst.GetAt(j)
                                    .ORPurchaseOrderLineRetList != null)
                                {
                                    for(int s = 0; s < retLst.GetAt(j)
                                        .ORPurchaseOrderLineRetList.Count; s++)
                                    {
                                        IORPurchaseOrderLineRet retLine = retLst
                                            .GetAt(j)
                                            .ORPurchaseOrderLineRetList.GetAt(s);
                                        if (!lineTxnIds.Contains(retLine
                                            .PurchaseOrderLineRet
                                            .TxnLineID.GetValue()))
                                        {
                                            lineTxnIds.Add(retLine
                                                .PurchaseOrderLineRet
                                                .TxnLineID.GetValue() + 
                                                "$%&" + retLst.GetAt(j)
                                                .TxnID.GetValue());
